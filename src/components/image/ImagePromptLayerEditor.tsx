...
import type { ImagePromptInsert, PromptBlock } from "@/types/imagePrompt";
import { DndContext, closestCenter, KeyboardSensor, PointerSensor, useSensor, useSensors } from '@dnd-kit/core';
import { SortableContext, sortableKeyboardCoordinates, verticalListSortingStrategy, useSortable } from '@dnd-kit/sortable';
import { CSS } from '@dnd-kit/utilities';
import { restrictToVerticalAxis } from '@dnd-kit/modifiers';

interface ImagePromptLayerEditorProps {
  categories: Category[];
      ...
      <Card className={`mb-4 ${isDragging ? 'border-blue-500' : ''}`}>
        <CardContent className="pt-4">
          ...
          <Input
            ...
            placeholder="Nome da camada (ex: Slide 1, Frame, etc)"
            className="mb-3"
          />
...
export const ImagePromptLayerEditor = ({ categories, onSubmit, onCancel }: ImagePromptLayerEditorProps) => {
  const [title, setTitle] = useState('');
  const [categoryId, setCategoryId] = useState('');
  const [selectedType, setSelectedType] = useState<'photo' | 'video' | 'design'>('photo');
  ...
  const handleAddLayer = () => {
    setLayers([
      ...layers,
      { label: `Slide ${layers.length + 1}`, text: '' }
    ]);
  };
    ...
    <Card className="p-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <RadioGroup
            value={selectedType}
            onValueChange={(value) => setSelectedType(value as 'photo' | 'video' | 'design')}
        ...
        <div>
          <label className="block text-sm font-medium mb-2">IA Recomendada (opcional)</label>
          <Input
            value={aiHint}
            onChange={(e) => setAiHint(e.target.value)}
            placeholder="Ex: Midjourney, DALL-E, Stable Diffusion, etc"
          />
        </div>
